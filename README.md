...
- 处理站点的反爬措施：隐藏章节链接（通过上一章/下一章探测解析）、懒加载图片、混淆图片域名，以及由 `chapterlog.js` 参数驱动的段落乱序还原
- 下载遵循站点限速（每分钟 15 次文本 / 10 次图片请求），因此大部头小说需要一些时间——与 [bili_novel_packer](https://github.com/montaro2017/bili_novel_packer) 项目节奏一致
